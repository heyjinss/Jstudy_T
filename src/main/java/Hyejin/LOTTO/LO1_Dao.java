...
public class LO1_Dao {
  ...
  /*
   * SELECT NO1, win, instr(win,'1,') 번호순당첨갯수, instr(BOUNUS,'1,') 보너스당첨횟수 FROM LOTO2 where no1 like
   * '%' || 's' || '%'// 숫자 검색 and NO1 BETWEEN 960 AND 968 //회차의 최소 및 최대 ORDER BY 번호순당첨횟수;
   * 
   */
  ...
  public ArrayList<LO1_List> list(SearchNumber s) { // 검색내용//DB서버에 넘겨줄 검색데이터
    ...
    // DATABASE연결은 현재 되지않으므로

    
    // 회차, 날짜, 당첨번호, 보너스번호, 검색번호, 카운트번호
     ...
     * 
     * System.out.print(s1.getNum()+"\t"); System.out.print(s1.getDate()+"\t");
     * System.out.print(s1.getWincode()+"\t"); System.out.print(s1.getBounus()+"\t");
